...
- BB automations and monitored watcher child threads provide durable wake mechanisms. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses them to re-check the finish condition on events or a heartbeat.
...
**Pitfall:** a duration is not a finish condition. "work on this for 4 hours" gives the agent nothing to check, and you'll wake up to four hours of motion instead of a result. Give the autonomous run a predicate that can pass or fail.
