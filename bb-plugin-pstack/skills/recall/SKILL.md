...
BB thread history is authoritative. Use `bb thread list --project "$BB_PROJECT_ID" --include-hidden --json`, `bb thread search <topic> --json`, and `bb thread log <thread-id> --all --format json`. Never read another project's threads unless the user asks.
...
3. Fan out across your BB thread history. List or search candidate threads, sort by `updatedAt`, exclude the current thread plus obvious eval and test noise, and divide the resulting thread IDs into slices. Call `pstack_get_model_config`, then spawn one worker per slice in one `pstack_spawn_threads` call with `preset: "general"`, `role: "swarm-workers"`, `readOnly: true`, and `workspace: "reuse"`. Each child reads only its assigned IDs with `bb thread log`, stops after relevant regions, and returns the same schema, one block per thread: topic, the user's goal, decisions, open work, struggles and corrections, and artifacts (PRs, tickets, branches), citing the BB thread ID. For one or two candidate threads, skip fan-out and search directly. Do not wait with a tool. Continue after BB's child-completion messages. Raw logs stay in the children and the main thread keeps only findings.
...
5. Verify against live state. A transcript or a stale ticket is history, not current truth, so take the PRs, branches, and tickets that the mining and the sweep surfaced and check them with `git` and `gh`. When the answer hinges on what an agent actually did (the tools it ran, files it read, errors it hit), read the full BB thread log, not just a trimmed summary.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write the brief through the **unslop** skill, cite chat findings by BB thread ID and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
