...
3. Fan out across your BB thread history. List or search candidate threads, sort by `updatedAt`, exclude the current thread plus obvious eval and test noise, and divide the resulting thread IDs into slices. Call `pstack_get_model_config`, then spawn one worker per slice in one `pstack_spawn_threads` call with `preset: "general"`, `role: "swarm-workers"`, `readOnly: true`, and `workspace: "reuse"`. Each child reads only its assigned IDs with `bb thread log`, stops after relevant regions, and returns the same schema, one block per thread: topic, the user's goal, decisions, open work, struggles and corrections, and artifacts (PRs, tickets, branches), citing the BB thread ID. For one or two candidate threads, skip fan-out and search directly. Collect with `pstack_collect_threads`; raw logs stay in the children and the main thread keeps only findings.
