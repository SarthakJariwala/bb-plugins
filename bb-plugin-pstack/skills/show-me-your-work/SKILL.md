...
At the end of the run, before handing back, check the log told the truth. Read only this run's BB thread log with `bb thread log --self --all --format json`; do not list or search unrelated threads. Walk the log against what actually happened:
