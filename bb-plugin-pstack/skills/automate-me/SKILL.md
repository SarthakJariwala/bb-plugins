---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via BB's skill-creator + unslop, optionally pulling fresh evidence from recent BB threads."
...
This skill orchestrates three others: an inline mining pass (see step 1), BB's `skill-creator` (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Inspect BB's skill catalog and `~/.bb/skills/*-mode/SKILL.md` for a mode matching the user's handle. If one exists, confirm intent in chat unless they already said "update my skill" or similar:
...
Mining misses intent that hasn't come up yet. Ask concise multiple-choice questions in chat rather than asking the user to invent an answer from scratch. Lower cognitive load, higher hit rate.
...
- **Child threads**: default, parallelism, model-to-task, specialized workflows.
...
Use BB's `skill-creator` skill to author the skill. Placement:

- Path: preserve an existing mode skill's location. For a new personal mode, use `~/.bb/skills/<handle>-mode/SKILL.md`, as `skill-creator` prescribes.
...
- BB's `skill-creator` skill: skill authoring process and writing guidelines.
