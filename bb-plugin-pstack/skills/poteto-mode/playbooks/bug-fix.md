...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_spawn_threads`, using `preset: "poteto-agent"`, `role: "bug-fix"`, a specific scope, and an isolated `new-worktree` when another writer is active. Collect with `pstack_collect_threads`; review the diff.
