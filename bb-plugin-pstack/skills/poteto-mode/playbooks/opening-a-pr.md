...
**Worktree.** Work from a git worktree off main. A child with `workspace: "reuse"` shares the parent's checkout; concurrent writers use separate `new-worktree` workspaces. Never run multiple writers against one branch. If serial workers intentionally reuse a branch, run `git fetch && git reset --hard origin/<branch>` between them. Dirty branch with unrelated work: patch out, fresh worktree, apply. Snarled worktree: reset from main, redo minimally.
...
A child thread that opens a PR runs `interrogate`, `/deslop` when available, and `/no-comments`. It returns the URL and does not babysit. Return to the parent.
