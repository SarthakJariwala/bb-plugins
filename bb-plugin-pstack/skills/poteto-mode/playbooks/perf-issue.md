...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_spawn_threads`, using `preset: "poteto-agent"`, `role: "perf-issue"`, and an isolated `new-worktree` when another writer is active. Do not wait with a tool. Review the diff after BB's child-completion message. Capture a post-fix trace.
