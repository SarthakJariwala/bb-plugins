...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <thread id>", "you're taking over", "pick up where X left off", a BB thread handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail from a BB thread ID, `bb thread search`, or a pushed branch. Read `bb thread show <id>` and the newest messages first, then scan the complete `bb thread log <id> --all --format json` for decision points. Parse a long log in a visible child with `pstack_spawn_threads`, using `preset: "general"`, `role: "swarm-workers"`, `readOnly: true`, and `workspace: "reuse"`; then keep only the reduced timeline in the main thread after BB's child-completion message (the **principle-guard-the-context-window** skill).
