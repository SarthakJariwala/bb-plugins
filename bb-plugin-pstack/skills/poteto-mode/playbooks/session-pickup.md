...
1. Locate the prior trail from a BB thread ID, `bb thread search`, or a pushed branch. Read `bb thread show <id>` and the newest messages first, then scan the complete `bb thread log <id> --all --format json` for decision points. Parse a long log in a hidden child with `pstack_spawn_threads`, using `preset: "general"`, `role: "swarm-workers"`, `readOnly: true`, and `workspace: "reuse"`; collect it with `pstack_collect_threads` and keep only the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
