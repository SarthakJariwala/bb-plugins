...
1. Identify the format and load it with the right tool. Parse large artifacts in a visible child with `pstack_spawn_threads`, using `preset: "general"`, `role: "swarm-workers"`, `readOnly: true`, and `workspace: "reuse"` (the **principle-guard-the-context-window** skill). Do not wait with a tool. Continue after BB's child-completion message. Keep only the reduced finding in the main thread.
