...
1. Identify the format and load it with the right tool. Parse large artifacts in a hidden child with `pstack_spawn_threads`, using `preset: "general"`, `role: "swarm-workers"`, `readOnly: true`, and `workspace: "reuse"` (the **principle-guard-the-context-window** skill). Collect it with `pstack_collect_threads`; keep only the reduced finding in the main thread.
