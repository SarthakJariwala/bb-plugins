...
3. Explore in visible children with one `pstack_spawn_threads` call. Use `preset: "poteto-agent"`, the role that matches each slice, `readOnly: true`, and `workspace: "reuse"` (the **guard-the-context-window** principle skill). Collect with `pstack_collect_threads`. Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
