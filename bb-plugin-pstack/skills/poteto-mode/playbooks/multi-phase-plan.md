...
3. Explore in visible children with one `pstack_spawn_threads` call. Use `preset: "poteto-agent"`, the role that matches each slice, `readOnly: true`, and `workspace: "reuse"` (the **guard-the-context-window** principle skill). Do not wait with a tool. Continue after BB's child-completion messages. Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
- [ ] Arm the 30-minute audit tick with a BB automation or a monitored terminal timer. Never leave the cadence to memory.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `openai-codex/gpt-5.6-sol` at `xhigh` reasoning at the PR head, per the boot recipe.
