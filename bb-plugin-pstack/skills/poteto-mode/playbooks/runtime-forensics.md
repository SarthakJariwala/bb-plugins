...
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, the loop firing without input. Parse large artifacts in a visible child with `pstack_spawn_threads`, using `preset: "general"`, `role: "swarm-workers"`, `readOnly: true`, and `workspace: "reuse"` (the **guard-the-context-window** principle skill). Do not wait with a tool. Continue after BB's child-completion message. Keep only the reduced finding in the main thread.
