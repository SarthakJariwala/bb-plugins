...
1. Call `pstack_spawn_threads` with one worker using `preset: "comment-sicko"`, `role: "judgment-prose"`, `readOnly: true`, and `workspace: "reuse"`. Pass only the scope; the preset carries Comment Sicko's rules. Do not wait with a tool. Continue after BB's child-completion message.
