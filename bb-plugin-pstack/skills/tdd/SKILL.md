...
- Call the code the way its users do and assert a literal expected value, per the **test-behavior-not-implementation** principle skill. If the test would still pass when every imported function returns `undefined`, rewrite the assertion or delete the test.
