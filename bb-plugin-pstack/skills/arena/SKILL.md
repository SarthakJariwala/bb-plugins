...
3. Pick the runners. Call `pstack_get_model_config` and use the `arena-runners` panel, one runner per entry. The panel defaults to four BB child threads and `/setup-pstack` can change its models, reasoning levels, and length. Spawn more only when the arena covers additional design directions. Repeating a selection is valid when the work is generation-bound rather than judgment-sensitive.
