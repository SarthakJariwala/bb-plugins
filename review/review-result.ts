...
function isLikelyFindingLine(line: string): boolean {
  ...
  return /^\s*(?:\*\*|__)?\[P[0-3]\](?:\*\*|__)?(?=\s|:|-)/i.test(line);
}
...
function hasNeedsAttentionVerdict(messageText: string): boolean {
  ...
  for (const line of lines) {
    ...
    if (inlineMatch && isNeedsAttentionVerdictValue(inlineMatch[1])) return true;
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = i + 1; j < lines.length; j++) {
      ...
      if (nextHeading) {
        ...
        if (/^(review scope|findings|fix queue|constraints(?:\s*&\s*preferences)?)\b:?/i.test(normalizedNextHeading)) break;
      }
...
// Kept behaviorally identical to the parser in ~/.pi/agent/extensions/review.ts.
export function hasBlockingReviewFindings(messageText: string): boolean {
  ...
  for (const line of candidateLines) {
    ...
    if (inCodeFence || !isLikelyFindingLine(line)) continue;
