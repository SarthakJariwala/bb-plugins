...
// Kept verbatim from ~/.pi/agent/extensions/review.ts.
...
function targetPrompt(target: ReviewTarget, includeLocalChanges: boolean): string {
  switch (target.type) {
    case "uncommitted":
      return UNCOMMITTED_PROMPT;
    case "baseBranch": {
      const basePrompt = BASE_BRANCH_PROMPT_FALLBACK.replace(/{branch}/g, target.branch);
    ...
    case "pullRequest": {
      const basePrompt = `First use \`gh pr view ${target.reference} --json number,title,baseRefName,headRefName\` to resolve the pull request metadata and use \`gh pr checkout ${target.reference}\` to check it out. Then follow this review instruction, substituting the resolved values for its placeholders:\n\n${PULL_REQUEST_PROMPT_FALLBACK}`;
      return includeLocalChanges ? `${basePrompt} ${LOCAL_CHANGES_REVIEW_INSTRUCTIONS}` : basePrompt;
    }
    case "folder":
...
export function buildReviewPrompt(
  target: ReviewTarget,
  options: { includeLocalChanges?: boolean; projectGuidelines?: string | null } = {},
): string {
  const prompt = targetPrompt(target, options.includeLocalChanges === true);
